...
public class BinaryTree<E> {
    ...
    public static void main(String[] args) {
        ...
        Node<Character> a = new Node<>('a'); //Node for A operand
        Node<Character> b = new Node<>('b'); //Node for B operand
        
        Node<Character> divide = new Node<>('/'); //Node for division operator

        divide.left = a; //Set a as division node's left child
        divide.right = b; //Set b as division node's right child

        Node<Character> multiply = new Node<>('*'); //Node for multiply operator
        multiply.left = plus; //Set plus operator as multiply's left child
        multiply.right = divide; //Set divide operator as multiply's right child

        exprBT2.root = multiply; //Set tree's root as multiply
        
        System.out.println(exprBT2.toString()); //Print this tree to a string

    }

    //Create a new StringBuilder, call preOrderTraverse method, and return the toString of the StringBuilder
    ...
    //Assess for null value. Then append the root, then the root's left values, and finally the root's right values.
    ...
    //Assess for null value. Then append the root's left values, then the root, and finally the root's right values.
    ...
    //Assess for null value. Then append the root's left values, then the root's right values, and finally the root.
    ...
    //Return the value given when Size method is called for the assigned root of given tree.
    public int size() {
        return size(root);
    }
    
    //Assess for null value. Proceed by calling root's left values, 
    //1 value to represent the root, and the root's right values.
    ...
    //Assess for null value. Proceed by finding maximum value among the root's left and right values, and add 1
    //to represent the root itself.
